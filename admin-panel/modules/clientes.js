...
export async function registrarCliente() {
  ...
  // Priorizar configuración inyectada (config.js) sobre defaults legacy
  const config = window.ADMIN_CONFIG || window.APP_CONFIG || {};
  const NOTIF_BASE = config.serverUrl || config.apiUrl || (window.__RAMPET__ && window.__RAMPET__.NOTIF_BASE) || '';
  const API_KEY = config.serverApiKey || config.apiKey || (window.__RAMPET__ && window.__RAMPET__.API_KEY) || '';
